...
module.exports = function (accounts, done, schema) {
  ...
  C.new(
    ...
    function run_(block, index) {
      // web3.eth.getBlockNumber().then(blockBumber => {
      //   console.log("Create contract " + schema.type + " at block number: " + blockBumber);
      // });
      ...
      promise
        ...
        .catch(function (error) {
          ...
          if (action.succeed) {
            return fail(
              index,
              action.on_error + ": " + error.toString().replace(/^error: /i, "")
            );
          }
